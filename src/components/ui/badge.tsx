...
// Two Figma components share one implementation because they differ only in
// scale: "Tag" (4:52) is the 11px parameter pill, "Status badge" (4:72) the
// 12px icon + text one. Both are tinted from the ramps; neither carries meaning
// in colour alone, which is why the status sizes always render an icon.
  ...
  "inline-flex w-fit shrink-0 items-center gap-1.5 rounded-pill border [&>svg]:pointer-events-none [&>svg]:size-3.5 [&>svg]:shrink-0",
  {
    variants: {
      tone: {
        ...
        // "Taking longer than expected": an outline rather than a fill, so a
        // job that is still running never reads as finished.
